import { executeQuery } from "@/lib/db"
import ProfileCard from "@/components/user/profile-card"
import { setupImageTables } from "@/app/actions/db-setup"

export default async function BrowsePage() {
  // Ensure the image tables exist before querying
  try {
    await setupImageTables()
  } catch (error) {
    console.error("Error setting up image tables:", error)
  }

  // Fetch users with safer query that doesn't assume profile_image_url exists
  const result = await executeQuery(
    `SELECT 
      u.id, 
      u.username,
      EXTRACT(YEAR FROM AGE(NOW(), u.date_of_birth)) as age
     FROM users u
     ORDER BY u.created_at DESC
     LIMIT 20`,
  )

  const users = result.rows.map((user) => ({
    ...user,
    profile_image_url: null, // Default to null since the column might not exist yet
  }))

  return (
    <div className="min-h-screen bg-gradient-to-b from-rose-50 to-white">
      {/* Header - Reusing the same header from layout */}
      <header className="container mx-auto px-4 py-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
              className="text-rose-500"
            >
              <path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z" />
            </svg>
            <h1 className="text-2xl font-bold text-gray-800">Forever Match</h1>
          </div>
          <div className="flex gap-4">
            <a href="/profile" className="text-rose-500 hover:text-rose-600 font-medium">
              My Profile
            </a>
          </div>
        </div>
      </header>

      <div className="container mx-auto py-8 px-4">
        <h1 className="text-3xl font-bold mb-8">Browse Profiles</h1>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {users.map((user) => (
            <ProfileCard key={user.id} user={user} />
          ))}

          {users.length === 0 && (
            <div className="col-span-full text-center py-12">
              <p className="text-gray-500">No profiles found</p>
            </div>
          )}
        </div>
      </div>

      {/* Footer */}
      <footer className="bg-gray-900 text-white py-4 mt-auto">
        <div className="container mx-auto px-4 text-center">
          <p>© {new Date().getFullYear()} Forever Match. All rights reserved.</p>
        </div>
      </footer>
    </div>
  )
}
