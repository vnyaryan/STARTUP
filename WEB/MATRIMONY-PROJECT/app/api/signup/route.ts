...
export async function POST(req: Request) {
  try {
    const { name, email, password, age, location, interests } = await req.json();
    ...
    const result = await sql`
      INSERT INTO users (name, email, password, age, location, interests)
      VALUES (${name}, ${email}, ${hashedPassword}, ${age}, ${location}, ${interests})
      RETURNING id;
    `;
